 ...
 * Features: Geo-Fencing, Payment Tracking, Admin Notifications, Token-based Auth
 */
...
/**
 * Geo-Fencing: List of allowed cities or areas
 * The backend will reject orders/services from outside these zones
 */
const ALLOWED_LOCATIONS = ['Kolkata', 'Delhi', 'Mumbai', 'Bangalore', 'Hyderabad'];
...
/**
 * Utility: Validate location against Geo-Fence
 */
function isLocationAllowed(location) {
  if (!location) return false;
  return ALLOWED_LOCATIONS.some(loc => location.toLowerCase().includes(loc.toLowerCase()));
}

/**
 * Private Action: Create Order with Address & Geo-Fencing
 */
function createOrder(data) {
  // Guard: Geo-Fence check
  if (!isLocationAllowed(data.address)) {
    return { success: false, message: `Sorry, we currently do not serve ${data.address}. We are only available in: ${ALLOWED_LOCATIONS.join(', ')}` };
  }

  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = getOrInsertSheet(ss, SHEET_NAMES.ORDERS, ['OrderID', 'Email', 'Items', 'Total', 'Status', 'PaymentStatus', 'Address', 'TransactionID', 'Date']);
  const orderId = 'ORD-' + Math.floor(1000 + Math.random() * 9000);
  
  // Default payment status to 'Unpaid' until record_payment is called
  sheet.appendRow([
    orderId, 
    data.userEmail, 
  ...
  sendEmailNotification(data.userEmail, 'Order Confirmed', `Order ${orderId} has been placed. Please complete payment if you haven't already.`);
...
/**
 * Private Action: Book Service with Address & Geo-Fencing
 */
function bookService(data) {
  if (!isLocationAllowed(data.address)) {
    return { success: false, message: `Service area restricted. We only serve: ${ALLOWED_LOCATIONS.join(', ')}` };
  }
  ...
  sendEmailNotification(data.userEmail, 'Service Request Received', `We received your request for ${data.category}. A service agent will contact you at ${data.address}.`);
...
function recordPayment(data) {
  ...
  for (let i = 1; i < values.length; i++) {
    if (values[i][0] === data.orderId) {
      // Update PaymentStatus (Col 6) and TransactionID (Col 8)
      sheet.getRange(i + 1, 6).setValue('Paid');
      sheet.getRange(i + 1, 8).setValue(data.transactionId || 'N/A');
